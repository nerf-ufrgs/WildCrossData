#VERIFICANDO AS IMAGENS FORNECIDAS PELA GALERA - UNDER
...
under <- map(.x = load, function(arquivo){
  read_excel(arquivo, #lê o arquivo excel que está no endereço em load
             ...
             range = cell_cols(1:6), #define as colunas que devem ser lidas
    ...
    select(1, 6) %>% #seleciona apenas as colunas 1 (id cameras) e coluna 3 (das fotos)
    separate(
      col = "Structure_photo",
...
checa_foto_faltando <- map2(set_names(names(NEST), nomes_img$V2), names(under), function(x, y){ #map para iterar entre todas as tabelinhas
  a <- NEST[[x]] #para cada um dos nomes (tabelas) de x, cria o objeto a
  b <- under[[y]] #para cada um dos nomes (tabelas) de y, cria o objeto b
  
  c <- a %>% 
    pull(nome_arq)
  
  d <- b %>% 
    filter(!is.na(Structure_photo)) %>% 
    pull(Structure_photo)
...
openxlsx::write.xlsx(x = checa_foto_faltando,
                     file = "Output/PLAN_ERROS_CHECA_IMAGEM_under2.xlsx", #salva o arquivo
                     asTable = TRUE)
...
NEST_under <- nomes_img$data %>% 
  lapply(., function(x) filter(x, V3 == "Under")) %>% 
  set_names(x = ., nm = nomes_img$V2) #separa as tabelas aninhadas e a cada uma delas dá o nome da sua respectiva planilha
...
checa_foto_sobrando <- map2(set_names(names(NEST_under), nomes_img$V2), names(under), function(x, y){ #map para iterar entre todas as tabelinhas
  a <- NEST_under[[x]] #para cada um dos nomes (tabelas) de x, cria o objeto a
  b <- under[[y]] #para cada um dos nomes (tabelas) de y, cria o objeto b
  ...
  openxlsx::write.xlsx(x = checa_foto_sobrando,
                       file = "Output/PLAN_ERROS_CHECA_IMAGEM_SOBRANDO_under.xlsx", #salva o arquivo
                       asTable = TRUE)
