  ...
  df |> 
    mutate(
      epsg = case_when(
        ...
        type == "Projected" & Datum == "WGS84" & hemis == "N" ~ 32600L + as.integer(zone),
        type == "Projected" & Datum == "WGS84" & hemis == "S" ~ 32700L + as.integer(zone),
        # projetados SIRGAS2000 / UTM (sul)
        type == "Projected" & Datum == "SIRGAS2000" & hemis == "S" ~ 31960L + as.integer(zone),
...
calc_nearest_osm_dist <- function(x, feature = c("highway", "railway", "man_made"), crs_metric = 3857, buffer = 1000, thresh = 50) {
  ...
  df <- x |> 
    ...
    dplyr::inner_join(osm_lines_sf |> sf::st_drop_geometry(), by = c("idx_nearest" = "rowid"))
  ...
  final_data <- df |>
    dplyr::mutate(distance_to = as.numeric(dists),
