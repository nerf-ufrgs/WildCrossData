#VERIFICANDO AS IMAGENS FORNECIDAS PELA GALERA - VERIFICANDO SE O NOME DOS ARQUIVOS CORRESPONDE AOS NOMES DOS ARQUIVOS NAS PLANILHAS
...
#CRIAMOS OBJETO PARA NOMEAR AS COLUNAS, POIS NAO EH POSSIVEL LER O NOME DAS COLUNAS DAS PLANILHAS DEVIDO A MESCLAGEM EXISTENTE NA ABA DAS ARMADILHAS FOTOGRAFICAS

#OBJETIVO PARA NOMEAR AS COLUNAS DA ABA DAS ARMADILHAS FOTOGRAFICAS
nome_colunas_af <- c("ID_Local_Camera", "Modelo_Camera", "Arquivo_Foto_Local_Camera")
#LENDO A PASTA ONDE ESTAO AS PLANILHAS (APENAS AS PLANILHAS)
excel <- list.files(
  path = "DATA_JANITOR/EXCEL_MOD", #fuça a pasta "EXCEL_MOD"
...
af <- map(.x = load, function(arquivo){
  read_excel(arquivo, #lê o arquivo excel que está no endereço em load
    ...
    select(1, 3) %>% #seleciona apenas as colunas 1 (id cameras) e coluna 3 (das fotos)
    separate(
      col = "Arquivo_Foto_Local_Camera",
      into = "Arquivo_Foto_Local_Camera",
      #sep = "\\.\\w{2,}g$"
...
imagens <- list.files(
  path = "GALERA_MOD", #fuça a pasta "GALERA"
...
checa_foto_faltando <- map2(set_names(names(NEST), nomes_img$V2), names(af), function(x, y){ #map para iterar entre todas as tabelinhas
  ...
  d <- b %>% 
    filter(!is.na(Arquivo_Foto_Local_Camera)) %>% 
    pull(Arquivo_Foto_Local_Camera)
  ...
  xlsx::write.xlsx(x = .,
                   file = "DATA_JANITOR/PLAN_ERROS_CHECA_IMAGEM.xlsx", #salva o arquivo
                   row.names = TRUE
  )
...
checa_foto_sobrando <- map2(set_names(names(NEST), nomes_img$V2), names(af), function(x, y){ #map para iterar entre todas as tabelinhas
  a <- NEST[[x]] #para cada um dos nomes (tabelas) de x, cria o objeto a
  b <- af[[y]] #para cada um dos nomes (tabelas) de y, cria o objeto b
  ...
  filter(!str_detect(string = ERRO, pattern = "character(0)")) %T>% #filtra os dados que não têm problema
  xlsx::write.xlsx(x = .,
                   file = "DATA_JANITOR/PLAN_ERROS_CHECA_IMAGEM_SOBRANDO.xlsx", #salva o arquivo
                   row.names = TRUE
  )
